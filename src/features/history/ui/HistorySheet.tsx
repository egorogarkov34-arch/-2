...
export function HistorySheet({ open, entries, onClose, onDelete, onClearAll }: Props) {
  ...
  return <AnimatePresence>{open && <>
    ...
    <motion.section className="bottom-sheet history-sheet" role="dialog" aria-modal="true" aria-label={t('entries')} initial={{ y: '105%' }} animate={{ y: 0 }} exit={{ y: '105%' }} transition={{ type: 'spring', stiffness: 320, damping: 31 }}>
      <div className="sheet-handle"/><div className="sheet-heading"><div><p className="eyebrow">{t('history')}</p><h2>{t('entries')}</h2></div><div className="history-heading-actions">{sortedEntries.length > 0 && !clearConfirmationOpen && <button className="history-clear" onClick={() => { haptic.tap(); setClearConfirmationOpen(true) }}>{t('deleteAll')}</button>}<button className="icon-button" onClick={close} aria-label={t('close')}><X size={20}/></button></div></div>
