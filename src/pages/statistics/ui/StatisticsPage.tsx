...
export default function StatisticsPage() {
  ...
  return <main className="page stats-page"><header className="page-header"><div><p className="eyebrow">{t('yourRhythm')}</p><h1>{t('stats')}</h1></div><button className="period-dropdown">2026 <ChevronDown size={16}/></button></header>
    ...
    <section className="chart-card line-card"><div className="section-row"><div><p className="eyebrow">{t('trend')}</p><h2>{t('waterBalance')}</h2></div><span className="soft-dot"/></div><div className="chart-holder line-holder"><ResponsiveContainer width="100%" height="100%"><LineChart data={data} margin={{ top: 8, right: 0, bottom: 0, left: -25 }}><CartesianGrid vertical={false} stroke="#FFFFFF0A"/><YAxis tickFormatter={(value: number) => `${value / 1000}L`} axisLine={false} tickLine={false} tick={{ fill: '#83878e', fontSize: 11 }}/><XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: '#83878e', fontSize: 11 }}/><Tooltip content={<WaterTooltip format={formatVolume}/>}/><Line type="monotone" dataKey="amount" stroke="#55AEFF" strokeWidth={3} dot={{ fill: '#0F1115', stroke: '#80C8FF', strokeWidth: 2, r: 4 }} activeDot={{ r: 6 }} isAnimationActive/></LineChart></ResponsiveContainer></div></section>
