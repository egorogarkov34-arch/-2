...
export interface HydrationConditions { temperatureC?: number | null; workoutMinutes?: number }
...
export function calculateWaterGoal(profile: Profile, conditions: HydrationConditions = {}): number {
  ...
  const workoutAdjustment = Math.min(1200, Math.max(0, conditions.workoutMinutes ?? 0) * 12)
