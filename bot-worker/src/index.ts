...
function parseReminderState(value: unknown): ReminderStatePayload | null {
  ...
  return { initData: payload.initData, dateKey: payload.dateKey, goal: Math.round(payload.goal), todayAmount: Math.round(payload.todayAmount), language: payload.language, reminders: { enabled: reminders.enabled, intervalMinutes: reminders.intervalMinutes, timeZone: normalizeTimeZone(reminders.timeZone) } }
}
...
async function handleReminderState(request: Request, env: Env) {
  ...
  const record: ReminderRecord = { userId: user.id, chatId: existing?.chatId ?? user.id, goal: payload.goal, todayAmount: payload.todayAmount, dateKey: payload.dateKey, language: payload.language, reminders: payload.reminders, reminderIndex: existing?.reminderIndex ?? 0, lastReminderAt: existing?.lastReminderAt, lastReminderDateKey: existing?.lastReminderDateKey, completedDateKey: existing?.completedDateKey, deliveryBlocked: false, updatedAt: Date.now() }
